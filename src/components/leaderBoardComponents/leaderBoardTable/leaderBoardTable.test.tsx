...
describe("LeaderBoardTable", () => {
  ...
  it("renders the table with correct headers", () => {
    render(<LeaderBoardTable {...mockLeaderBoardTableProps} />);
    expect(
      screen.getByTestId("leaderboard-position-header"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("leaderboard-player-name-header"),
    ).toBeInTheDocument();
    expect(
      screen.getByTestId("leaderboard-finish-time-header"),
    ).toBeInTheDocument();
  });

  it("handles empty state", () => {
    render(
      <LeaderBoardTable {...mockLeaderBoardTableProps} leaderBoardData={[]} />,
    );
  ...
  describe("Renders with correct table row data", () => {
    ...
    it("renders table row 2 with second entry from mockProps", () => {
      ...
      expect(
        entryRowTwo.firstChild?.nextSibling?.nextSibling,
  ...
  it("renders the correct number of rows", () => {
    ...
    expect(rows.length).toBe(
      mockLeaderBoardTableProps.leaderBoardData.length + 1,
    ); // +1 for header row
  });
});
